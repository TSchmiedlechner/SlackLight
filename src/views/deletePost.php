...
<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header">
            Delete post
            <small>from channel <i><?php echo $channel->getName() ?></i></small>
        </h1>
    </div>
</div>
<!-- /.row -->
    ...
    <div class="row">
        <div class="col-lg-8">
            <form method="post" id="addNewPost"
                  action="<?php echo Util::action(Controller::ACTION_DELETEPOST, array('view' => 'channel', 'id' => $channel->getId())); ?>">
                <div>
                    <input type="text" class="form-control" name="title" placeholder="Title" id="postTitle"
                           value="<?php echo $post->getTitle() ?>" disabled>
                    <textarea class="form-control" rows="3" placeholder="Text" name="text" id="postText"
                              disabled><?php echo $post->getText() ?></textarea>
